...
namespace Avalonia.WebRender.Utils
{
    public static class UnitExtensions
    {
        ...
        {
            return new ColorF { R = color.R / 255.0f, G = color.G / 255.0f, B = color.B / 255.0f, A = color.A / 255.0f };
        }

        public static LayoutRect ToLayoutRect(this Rect rect)
        {
            return new LayoutRect { 
                origin = rect.Position.ToLayoutPoint(),
                size = rect.Size.ToLayoutSize()
            };
        }

        public static LayoutSize ToLayoutSize(this Size size)
        {
            return new LayoutSize { width = (float)size.Width, height = (float)size.Height };
        }

        public static LayoutPoint ToLayoutPoint(this Point point)
        {
            return new LayoutPoint { x = (float)point.X, y = (float)point.Y };
        }

        // SkiaSharp extensions. Used for text layout and path rendering
