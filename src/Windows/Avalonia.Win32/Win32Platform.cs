...
namespace Avalonia.Win32
{
    class Win32Platform : IPlatformThreadingInterface, IPlatformSettings, IWindowingPlatform, IPlatformIconLoader
    {
        ...
        public static void Initialize(bool deferredRendering = true, bool useAngle = true)
        {
            ...
            if (useAngle) {
                AvaloniaLocator.CurrentMutable
                    .Bind<Func<GlRequest, IGlContextBuilder>>()
                    .ToConstant<Func<GlRequest, IGlContextBuilder>>(
                        (request) => new EglContextBuilder(request)
                    );
            }
            else
            {
                throw new NotImplementedException("WGL is not yet implemented!");
            }
